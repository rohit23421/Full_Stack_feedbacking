...
async function submitFeedback(
  ...
  try {
    ...
    //DISMISS LOADING TEXT AND SHOW SUCCESS TEXT
...
export default function NewFeedbackPage() {
  const router = useRouter(); //using for useeffect hook for redirecting, rerouting user
  const [state, action, isPending] = useActionState(submitFeedback, {
    success: false,
    error: "",
  });

  //REDIRECT ON SUCCESS OF SUBMITTING FEEDBACK USING USEEFFECT
  useEffect(() => {
    if (state.success) {
      const timer = setTimeout(() => {
        router.push("/feedback");
        router.refresh();
      }, 1500); //waiting fo 1500ms for the toast to be visible
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" render={<Link href="/feedback" />}>
